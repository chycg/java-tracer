package com.ali.trace.agent.jetty.handler;
...
import com.ali.trace.agent.core.ConfigPool;
import com.ali.trace.agent.core.ConfigPool.LoaderSet;
import com.ali.trace.agent.jetty.vo.DataRet;
import com.ali.trace.agent.util.XmlNode;
...
public class ClassHandler implements ITraceHttpHandler {
    ...
    public DataRet<String> redefine(@TraceParam("lid")String loaderId, @TraceParam("cname")String cname) throws IOException {
        ...
        try {
			...
			ret = new DataRet<String>(true, 0, "redefine class[" + cname + "]lid[" + loaderId + "]loader[" + loader + "]");
        } catch (Exception e) {
			ret = new DataRet<String>(false, -1, "redefine class[" + cname + "]lid[" + loaderId + "]failed" + e.getMessage());
        }
        return ret;
    }
    ...
    public DataRet<String> redefineType(@TraceParam("type")String type) throws IOException {
        ...
        try {
			...
			ret = new DataRet<String>(true, 0, "redefine type failed");
        } catch (Exception e) {
			ret = new DataRet<String>(false, -1, "redefine type failed" + e.getMessage());
        }
        return ret;
    }
